...
- 后续刷新复用最近一次完整覆盖留下的可信提交边界；按时间倒序的来源在重新遇到该 submission ID 且继续读到严格更早的时间后即可停止分页。可信 ID 消失或时间变化时自动回退原有时间下界；AtCoder 继续使用其原生 `from_second` 游标；VJudge 按 `runId` 窗口分页，提交时间不用于排序或边界剪枝。
